# Vendored from https://github.com/shiyu-coder/Kronos (MIT License, see ./LICENSE),
# commit at time of vendoring: master branch, model/kronos.py.
# Changes from upstream:
#   1. Absolute `from model.module import *` + `sys.path.append("../")` replaced
#      with a relative import so this works as a subpackage of analyst_layer.
#   2. Added `return_all_samples` to auto_regressive_inference() (default False,
#      existing behavior unchanged) plus additive KronosPredictor.generate_paths()/
#      predict_paths() methods, so callers can get the individual sample_count
#      paths instead of only their mean — needed for path-level statistics
#      (touch probability, dispersion) that predict()/generate() can't provide.
#      predict()/generate() themselves are untouched other than extracting the
#      shared DataFrame-building tail into _finish_predict().
import numpy as np
import pandas as pd
import torch
...
def auto_regressive_inference(tokenizer, model, x, x_stamp, y_stamp, max_context, pred_len, clip=5, T=1.0, top_k=0, top_p=0.99, sample_count=5, verbose=False, return_all_samples=False):
    with torch.no_grad():
        ...
        if return_all_samples:
            # Deviation from upstream: skip the mean-across-samples so callers can
            # compute per-path statistics (e.g. touch probabilities, dispersion)
            # instead of only ever seeing the sample-averaged path.
            return preds  # shape: (batch, sample_count, pred_len, features)
...
class KronosPredictor:
    ...
    def generate_paths(self, x, x_stamp, y_stamp, pred_len, T, top_k, top_p, sample_count, verbose):
        """Like generate(), but returns every one of the sample_count sampled
        paths instead of their mean — shape (batch, sample_count, pred_len, features).
        Added on top of upstream Kronos for shadow-signal path-level statistics
        (touch probability, dispersion) that the mean-only predict()/generate()
        can't provide.
        """
        x_tensor = torch.from_numpy(np.array(x).astype(np.float32)).to(self.device)
        x_stamp_tensor = torch.from_numpy(np.array(x_stamp).astype(np.float32)).to(self.device)
        ...
        preds = auto_regressive_inference(self.tokenizer, self.model, x_tensor, x_stamp_tensor, y_stamp_tensor, self.max_context, pred_len,
                                          self.clip, T, top_k, top_p, sample_count, verbose, return_all_samples=True)
        preds = preds[:, :, -pred_len:, :]
        return preds

    def predict_paths(self, df, x_timestamp, y_timestamp, pred_len, T=1.0, top_k=0, top_p=0.9, sample_count=1, verbose=True):
        """Like predict(), but returns a list of per-path DataFrames (length
        sample_count) instead of one sample-averaged DataFrame.
        """
        ...
        preds = self.generate_paths(x, x_stamp, y_stamp, pred_len, T, top_k, top_p, sample_count, verbose)
        preds = preds.squeeze(0)  # (sample_count, pred_len, features)
        preds = preds * (x_std + 1e-5) + x_mean

        return [self._finish_predict(preds[i], y_timestamp) for i in range(preds.shape[0])]
    ...
    def predict(self, df, x_timestamp, y_timestamp, pred_len, T=1.0, top_k=0, top_p=0.9, sample_count=1, verbose=True):
        ...
        preds = preds * (x_std + 1e-5) + x_mean
        return self._finish_predict(preds, y_timestamp)

    def _finish_predict(self, preds, y_timestamp):
